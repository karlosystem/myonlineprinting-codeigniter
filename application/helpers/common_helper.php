...
function get_values($p_id, $sp_id, $att_id, $counter)
{
	...
	$str = '';
	$str .= '<select name="value_array[]" id="value_array_' . $counter . '"   class="form-select bg-success text-white">';
	if ($result) {
		foreach ($result as $r) {
...
function get_attr_pricing($qty = 0, $p_id = 0, $sp_id = 0, $comb = "")
{
	...
	foreach ($result as $row) {
		...
		$str .= '<td><a href="' . base_url() . 'cart/basket/' . $row["pricing_id"] . '" class="btn btn-primary">Add to cart</a></td>';
	}
	return  $str;
}
